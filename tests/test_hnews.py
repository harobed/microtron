...
class TestHNews1(unittest2.TestCase):
    def setUp(self):
    ...
    def assertSubsetEqual(self, ob1, ob2):
        ...
        self.assert_(
            isinstance(ob1, type(ob2)),
            " -> type() differs: %s != %s" % (type(ob1), type(ob2))
        )

        if isinstance(ob2, list):
            self.assertEqual(
                len(ob1), len(ob2),
                " -> len() differs: %s != %s" % (len(ob1), len(ob2))
            )
    ...
    def test_hnews1(self):
        ...
        expected = [{
            ...
            'entry-title': 'Renewing US ties with Latin America, Obama says he\
                            wants to lead, not lecture, on democracy',
            'source-org': {
                '__type__': 'vcard',
                'fn': 'Associated Press',
            ...
            'updated': {
                'datetime': datetime.datetime(
                    2009, 4, 19, 18, 17, 29, 0, isodate.tzinfo.Utc()
                )
            }
        }]
